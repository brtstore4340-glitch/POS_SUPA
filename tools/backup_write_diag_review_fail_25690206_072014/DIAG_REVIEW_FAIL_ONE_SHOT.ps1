...
# PURPOSE: Collect full evidence for vitest/vite failures with verbose stderr. No repo modifications.
param(
...
function Log([string]$Line){ $t=(Get-Date).ToString("s"); Add-Content -LiteralPath $logFile -Value ("[{0}] {1}" -f $t,$Line) -Encoding utf8 }
...
function Invoke-CmdLine([string]$Title,[string]$CmdLine,[string]$WorkDir,[int]$TimeoutSec){
  ...
  $p.StartInfo=$pinfo
  [void]$p.Start()
  if(-not $p.WaitForExit($TimeoutSec*1000)){ try{ $p.Kill($true) }catch{}; Log ("[FAIL] Timeout after {0}s: {1}" -f $TimeoutSec,$Title); return @{ok=$false;code=124;title=$Title;out="";err="TIMEOUT"} }
...
if(-not (Test-Path -LiteralPath $pkg -PathType Leaf)){ Log "[FATAL] Missing package.json"; throw "Missing package.json" }
$pkgRaw=Get-Content -LiteralPath $pkg -Raw
$pkgObj=$null
try{ $pkgObj=$pkgRaw | ConvertFrom-Json }catch{ $pkgObj=$null }
Log "[INFO] package.json loaded"
if($pkgObj -and $pkgObj.scripts){ $names=$pkgObj.scripts.PSObject.Properties.Name; Log ("[INFO] scripts: " + ($names -join ", ")) } else { Log "[WARN] scripts missing/unreadable" }

$binDir=Join-Path $repo "node_modules\.bin"
$viteCmd=Join-Path $binDir "vite.cmd"
$vitestCmd=Join-Path $binDir "vitest.cmd"
function BinInfo([string]$p){ if(Test-Path -LiteralPath $p -PathType Leaf){ $fi=Get-Item -LiteralPath $p; return ("FOUND ({0} bytes): {1}" -f $fi.Length,$p) } return ("MISSING: {0}" -f $p) }
...
[void]$sb.AppendLine("- vitest.cmd: " + (BinInfo $vitestCmd))
[void]$sb.AppendLine("- vite.cmd: " + (BinInfo $viteCmd))
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Results (summary)")
[void]$sb.AppendLine(("- Vitest exit code: {0}" -f $testRes.code))
[void]$sb.AppendLine(("- Vite exit code: {0}" -f $buildRes.code))
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Vitest stderr (tail)")
[void]$sb.AppendLine($fence)
$tailTest = ($testRes.err -split "`r?`n") | Select-Object -Last 140
[void]$sb.AppendLine(($tailTest -join "`r`n"))
[void]$sb.AppendLine($fence)
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Vite stderr (tail)")
[void]$sb.AppendLine($fence)
$tailBuild = ($buildRes.err -split "`r?`n") | Select-Object -Last 180
