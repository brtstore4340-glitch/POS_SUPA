...
# OUTPUTS: tools/logs/review_one_shot_*.log, tools/review/REVIEW_*.md, tools/LAST_BACKUP_DIR.txt
param(
...
if (Test-Path -LiteralPath $pkg -PathType Leaf) {
  ...
  if ($scripts -contains "test")  { $cmdResults += Invoke-Cmd "Tests" "npm" @("run","test")  $repo $TimeoutSec } else { Add-N "No script 'test' in package.json." }
  if ($scripts -contains "build") { $cmdResults += Invoke-Cmd "Build" "npm" @("run","build") $repo $TimeoutSec } else { Add-N "No script 'build' in package.json." }
}
