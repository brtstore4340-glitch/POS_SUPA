# FILE: FIX_REVIEW_PROJECT_FULL_REWRITE_V2.ps1
# PURPOSE: Overwrite REVIEW_PROJECT_FULL.ps1 with a parser-safe version (no literal ``` in AppendLine) + backup + logs.
# RUN:
#   pwsh -ExecutionPolicy Bypass -File .\FIX_REVIEW_PROJECT_FULL_REWRITE_V2.ps1 -RepoRoot "D:\01 Main Work\Boots\Boots-POS Gemini"
...
$log = Join-Path $logsDir ("fix_review_rewrite_v2_{0}.log" -f $ts)
Write-Utf8NoBom $log ("[INFO] RepoRoot: {0}`n[INFO] Start: {1}`n" -f $repo, (Get-Date).ToString("s"))

$backupDir = Join-Path $toolsDir ("backup_fix_review_rewrite_v2_{0}" -f $ts)
New-Dir $backupDir
...
if (Test-Path -LiteralPath $target -PathType Leaf) {
  ...
  Add-Content -LiteralPath $log -Value ("[WARN] REVIEW_PROJECT_FULL.ps1 not found; will create new.") -Encoding utf8
}

# NOTE: Single-quoted here-string, no nesting, terminator is on its own line.
$fixed = @'
...
# Parser-safe fence (avoid literal ``` in method calls)
$Fence = ('`' * 3)
...
if ($findingsMust.Count -gt 0 -or $hardFail) { [void]$rep.AppendLine("- Verdict: **NEEDS FIX**") }
else { [void]$rep.AppendLine("- Verdict: **PASS**") }
...
else { $i=0; foreach ($r in $cmdResults) { $i++; [void]$rep.AppendLine(("- #{0}: ok={1} code={2}" -f $i, $r.ok, $r.code)) } }
...
Add-Content -LiteralPath $log -Value ("[PASS] Wrote fixed REVIEW_PROJECT_FULL.ps1: {0}" -f $target) -Encoding utf8
