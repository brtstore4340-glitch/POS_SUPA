# FILE: FIX_REVIEW_PROJECT_FULL_REWRITE.ps1
# PURPOSE: Rewrite REVIEW_PROJECT_FULL.ps1 to a parser-safe version (no literal ``` in method calls) + backup + logs.
# RUN:
#   pwsh -ExecutionPolicy Bypass -File .\FIX_REVIEW_PROJECT_FULL_REWRITE.ps1 -RepoRoot "D:\01 Main Work\Boots\Boots-POS Gemini"
...
$log = Join-Path $logsDir ("fix_review_rewrite_{0}.log" -f $ts)
Write-Utf8NoBom $log ("[INFO] RepoRoot: {0}`n[INFO] Start: {1}`n" -f $repo, (Get-Date).ToString("s"))

$backupDir = Join-Path $toolsDir ("backup_fix_review_rewrite_{0}" -f $ts)
New-Dir $backupDir
...
if (Test-Path -LiteralPath $target -PathType Leaf) {
  ...
  Add-Content -LiteralPath $log -Value ("[INFO] Backed up existing REVIEW_PROJECT_FULL.ps1 -> {0}" -f $backupDir) -Encoding utf8
} else {
  Add-Content -LiteralPath $log -Value ("[WARN] No existing REVIEW_PROJECT_FULL.ps1 found; will create new.") -Encoding utf8
}

# --- Write the fixed REVIEW_PROJECT_FULL.ps1 (parser-safe) ---
$fixed = @'
...
# IMPORTANT: Avoid literal ``` in method calls to prevent parser edge cases.
$Fence = ('`' * 3)
...
if (Test-Path -LiteralPath $pkg) { Add-Evidence "package.json (scripts inspected)" }
else { Add-Must "Missing package.json at repo root (cannot verify build reliably)" }

# Firestore rules quick scan (safe posture)
...
# Secrets risk scan (names only, no values)
...
if ($findingsMust.Count -gt 0 -or $hardFail) { [void]$rep.AppendLine("- Verdict: **NEEDS FIX** (must-fix present or verify failed)") }
else { [void]$rep.AppendLine("- Verdict: **PASS** (no must-fix found by this runner)") }

[void]$rep.AppendLine("")
[void]$rep.AppendLine("## Findings — Must-fix")
if ($findingsMust.Count -eq 0) { [void]$rep.AppendLine("- (none)") }
else { foreach ($f in $findingsMust) { [void]$rep.AppendLine("- " + $f) } }

[void]$rep.AppendLine("")
[void]$rep.AppendLine("## Findings — Should-fix")
if ($findingsShould.Count -eq 0) { [void]$rep.AppendLine("- (none)") }
else { foreach ($f in $findingsShould) { [void]$rep.AppendLine("- " + $f) } }

[void]$rep.AppendLine("")
[void]$rep.AppendLine("## Findings — Nice-to-have")
if ($findingsNice.Count -eq 0) { [void]$rep.AppendLine("- (none)") }
else { foreach ($f in $findingsNice) { [void]$rep.AppendLine("- " + $f) } }

[void]$rep.AppendLine("")
[void]$rep.AppendLine("## Command Results (summary)")
if ($cmdResults.Count -eq 0) { [void]$rep.AppendLine("- (no commands were run)") }
else {
  $i = 0
  foreach ($r in $cmdResults) { $i++; [void]$rep.AppendLine(("- #{0}: ok={1} code={2}" -f $i, $r.ok, $r.code)) }
}
...
Write-Utf8NoBom $target $fixed
Add-Content -LiteralPath $log -Value ("[PASS] Rewrote: {0}" -f $target) -Encoding utf8
Add-Content -LiteralPath $log -Value ("[PASS] EXIT CODE: 0") -Encoding utf8
exit 0
'@
