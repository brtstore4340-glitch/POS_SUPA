COPILOT_CHAT_ONLY_SILENT.ps1param(
  [switch]$AlsoApplyToInsiders,
  [switch]$InstallProfileFunction
)
...
$backupDir = JP $cwd ("tools\backup_copilot_chat_only_silent_{0}" -f $ts)
New-Dir $backupDir
Write-Utf8NoBom -Path (JP $cwd "tools\LAST_BACKUP_DIR.txt") -Content ($backupDir + "`n")

$logPath = JP $cwd ("tools\logs\copilot_chat_only_silent_{0}.log" -f $ts)
...
try {
  ...
  $targets = @((JP $appdata "Code\User\settings.json"))
  ...
  foreach ($settingsPath in $targets) {
    ...
    # Keep Copilot Chat, but silence everything else
    ...
    # Reduce chat “noise” + system notifications
    $obj["chat.detectParticipant.enabled"] = $false
    $obj["chat.notifyWindowOnNeedInput"] = $false
    $obj["chat.notifyWindowOnResponseReceived"] = $false
    $obj["chat.notifyWindowOnConfirmation"] = $false

    # Keep chat menu available (so you can still open chat)
    if (-not $obj.ContainsKey("chat.commandCenter.enabled")) {
      $obj["chat.commandCenter.enabled"] = $true
    }

    Write-Json -Path $settingsPath -Obj $obj
    Write-Host "[OK] Updated settings: keep Copilot Chat, disable suggestions/agent/notifications" -ForegroundColor Green
  }

  if ($InstallProfileFunction) {
    $profilePath = $PROFILE.CurrentUserAllHosts
    $profileDir = Split-Path -Parent $profilePath
    if (-not (Test-Path -LiteralPath $profileDir)) { New-Dir $profileDir }

    $bk2 = Backup-File -Path $profilePath -BackupDir $backupDir
    if ($bk2) { Write-Host "[INFO] Backed up profile -> $bk2" }
...
function Open-CopilotChat {
  ...
  if (-not $cmd) { throw "Missing 'code' on PATH" }

  # VS Code CLI supports: code chat -m ask [prompt]
  # If prompt is empty, send a single space to force-open in some builds.
  $p = $Prompt
  if ([string]::IsNullOrWhiteSpace($p)) { $p = " " }

  & code chat -m ask -r --maximize $p
}
'@

    if (-not (Test-Path -LiteralPath $profilePath)) {
      Write-Utf8NoBom -Path $profilePath -Content ($fn + "`n")
    } else {
      $raw = Get-Content -LiteralPath $profilePath -Raw
      if ($raw -notmatch "function\s+Open-CopilotChat") {
        Write-Utf8NoBom -Path $profilePath -Content ($raw.TrimEnd() + "`n`n" + $fn + "`n")
      }
    }

    Write-Host "[OK] Installed Open-CopilotChat into PowerShell profile" -ForegroundColor Green
  }

  Write-Host "[DONE] Close ALL VS Code windows, then reopen." -ForegroundColor Cyan
  Write-Host "[NOTE] Open chat: Ctrl+Alt+I (workbench.action.chat.open) or run: code chat -m ask 'hi'" -ForegroundColor Cyan
  $exit = 0
} catch {
