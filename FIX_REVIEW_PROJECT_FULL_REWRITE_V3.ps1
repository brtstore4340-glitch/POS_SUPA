# FILE: FIX_REVIEW_PROJECT_FULL_REWRITE_V3.ps1
# PURPOSE: Rewrite REVIEW_PROJECT_FULL.ps1 with a guaranteed parser-safe method (no here-strings), backup+logs, UTF-8 no BOM.
# RUN:
#   pwsh -ExecutionPolicy Bypass -File .\FIX_REVIEW_PROJECT_FULL_REWRITE_V3.ps1 -RepoRoot "D:\01 Main Work\Boots\Boots-POS Gemini"
...
$log = Join-Path $logsDir ("fix_review_rewrite_v3_{0}.log" -f $ts)
Write-Utf8NoBom $log ("[INFO] RepoRoot: {0}`n[INFO] Start: {1}`n" -f $repo, (Get-Date).ToString("s"))

$backupDir = Join-Path $toolsDir ("backup_fix_review_rewrite_v3_{0}" -f $ts)
New-Dir $backupDir
...
# Build file content as an array of lines (NO here-strings)
...
A ''
A '$Fence = ("`" * 3)'   # triple backtick, parser-safe
A ''
A '# Context pack'
...
LogLine $log ("[PASS] Rewrote (no-here-string): {0}" -f $target)

# Quick syntax check (parse only)
try {
  & pwsh -NoProfile -Command "Set-StrictMode -Off; . '$target' -RepoRoot '$repo' -MaxFiles 1 -MaxFileKB 1 -TimeoutSec 1" | Out-Null
  LogLine $log "[PASS] Smoke parse/entry OK"
} catch {
  LogLine $log ("[WARN] Smoke run returned: {0}" -f $_.Exception.Message)
}
