# FILE: WRITE_REVIEW_ONE_SHOT_SAFE_V3.ps1
# PURPOSE: Rewrite REVIEW_ONE_SHOT.ps1 using cmd.exe /c for reliable npm args, backup+logs, then run review.
# RUN:
#   pwsh -ExecutionPolicy Bypass -File .\WRITE_REVIEW_ONE_SHOT_SAFE_V3.ps1 -RepoRoot "D:\01 Main Work\Boots\Boots-POS Gemini"
...
$writerLog = Join-Path $logsDir ("write_review_one_shot_v3_{0}.log" -f $ts)
...
$backupDir = Join-Path $toolsDir ("backup_write_review_one_shot_v3_{0}" -f $ts)
New-Dir $backupDir
...
A '  if ($pm -eq "npm")      { $cmdResults += Invoke-CmdLine "Install (npm ci)"  "npm ci" $repo $TimeoutSec }'
A '  elseif ($pm -eq "pnpm") { $cmdResults += Invoke-CmdLine "Install (pnpm i)" "pnpm i --frozen-lockfile" $repo $TimeoutSec }'
A '  else                    { $cmdResults += Invoke-CmdLine "Install (yarn install)" "yarn install --frozen-lockfile" $repo $TimeoutSec }'
A '} else { Log "[INFO] node_modules exists; skipping install." }'
A ''
A 'if ($scripts -contains "test")  {'
A '  if ($pm -eq "npm")      { $cmdResults += Invoke-CmdLine "Tests" "npm run test" $repo $TimeoutSec }'
A '  elseif ($pm -eq "pnpm") { $cmdResults += Invoke-CmdLine "Tests" "pnpm run test" $repo $TimeoutSec }'
A '  else                    { $cmdResults += Invoke-CmdLine "Tests" "yarn test" $repo $TimeoutSec }'
A '} else { Add-N "No script ''test'' in package.json." }'
A ''
A 'if ($scripts -contains "build") {'
A '  if ($pm -eq "npm")      { $cmdResults += Invoke-CmdLine "Build" "npm run build" $repo $TimeoutSec }'
A '  elseif ($pm -eq "pnpm") { $cmdResults += Invoke-CmdLine "Build" "pnpm run build" $repo $TimeoutSec }'
A '  else                    { $cmdResults += Invoke-CmdLine "Build" "yarn build" $repo $TimeoutSec }'
A '} else { Add-N "No script ''build'' in package.json." }'
A ''
A '$hardFail = $false'
