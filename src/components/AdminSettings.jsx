...
export default function AdminSettings({ onClose, onBack, variant = 'page' }) {
    ...
    >
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          ...
          {handleClose && (
            <button onClick={handleClose} className="p-2 hover:bg-slate-200 rounded-full transition-colors"><X size={20} /></button>
          )}
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          {handleClose && (
            <button onClick={handleClose} disabled={uploadStatus === 'uploading'} className="px-6 py-2 rounded-lg text-slate-600 hover:bg-slate-200 font-bold transition-colors disabled:opacity-50">Close</button>
          )}
          ...
          {uploadStatus === 'done' && (
            <button onClick={handleClose} className="px-6 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white font-bold shadow-md flex items-center gap-2">
