...
trap { try { if ($script:LogFile) { Add-Content -LiteralPath $script:LogFile -Value ("[FATAL] " + $_.Exception.Message) -Encoding utf8; Add-Content -LiteralPath $script:LogFile -Value "[FATAL] EXIT CODE: 1" -Encoding utf8 } } catch {}; exit 1 }
...
function Get-RepoScripts([string]$PkgPath){ try{ $json=Get-Content -LiteralPath $PkgPath -Raw | ConvertFrom-Json; if ($null -ne $json.scripts){ return $json.scripts.PSObject.Properties.Name } }catch{}; return @() }
...
if (-not (Test-Path -LiteralPath $pkg -PathType Leaf)){ Add-M "Missing package.json at repo root."; } else { Add-E "package.json found" }
$scripts=@()
if (Test-Path -LiteralPath $pkg -PathType Leaf){ $scripts=Get-RepoScripts $pkg; Add-E ("package.json scripts: " + ($scripts -join ", ")) }

# Ensure deps are sane: if vitest/vite missing in node_modules/.bin, run npm ci
$binDir=Join-Path $repo "node_modules\.bin"
$viteCmd=Join-Path $binDir "vite.cmd"
$vitestCmd=Join-Path $binDir "vitest.cmd"
$needCi=$false
if (-not (Test-Path -LiteralPath $viteCmd -PathType Leaf)) { $needCi=$true; Log "[WARN] Missing node_modules/.bin/vite.cmd" }
if (-not (Test-Path -LiteralPath $vitestCmd -PathType Leaf)) { $needCi=$true; Log "[WARN] Missing node_modules/.bin/vitest.cmd" }
if ($needCi) {
  Log "[INFO] Repairing deps with: npm ci"
  [void](Invoke-CmdLine "Install (npm ci)" "npm ci" $repo $TimeoutSec)
} else { Log "[INFO] Local bins present; skipping npm ci." }

$cmdResults=@()
if ($scripts -contains "test")  { $cmdResults += Invoke-CmdLine "Tests" "npm exec -- vitest run --watch=false --reporter=verbose" $repo $TimeoutSec }
if ($scripts -contains "build") { $cmdResults += Invoke-CmdLine "Build" "npm exec -- vite build" $repo $TimeoutSec }

$hardFail=$false
foreach($r in $cmdResults){ if(-not $r.ok){ $hardFail=$true } }
if ($hardFail) { Add-M "One or more verify commands failed (see log for stderr)." }
